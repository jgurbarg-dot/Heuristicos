...
def traducir_texto(texto):
    """Traduce con Google y usa MyMemory como alternativa si Google bloquea."""
    texto_recortado = texto[:4000]
    
    # Intento 1: Google Translator
    try:
        traductor = GoogleTranslator(source='auto', target='es')
        return traductor.translate(texto_recortado)
    except Exception:
        pass

    # Intento 2: MyMemory (Respaldo)
    try:
        traductor_alt = MyMemoryTranslator(source='auto', target='es')
        return traductor_alt.translate(texto_recortado[:500])
    except Exception as e:
        return f"No se pudo traducir automáticamente por saturación de servidores.\n\nTexto original:\n{texto}"
...
if "traducir_actual" not in st.session_state:
    st.session_state.traducir_actual = False
...
if st.session_state.fuente_elegida is None:
    ...
    palabra_busqueda = st.text_input(f"🔍 Ingresa un término en español o inglés (ej. bomba, destilación, heat, pump):", key="campo_busqueda")
    ...
    if palabra_busqueda:
        ...
        if resultados:
            ...
            # Muestra texto original o traducido según el botón
            if st.session_state.traducir_actual:
                with st.spinner("Traduciendo..."):
                    texto_mostrar = traducir_texto(heuristico_actual["texto"])
                st.text_area(label="Heurístico (Español):", value=texto_mostrar, height=240)
            else:
                st.text_area(label="Heurístico Completo (Original):", value=heuristico_actual["texto"], height=240)

            # Botón para activar la traducción del elemento actual
            if not st.session_state.traducir_actual:
                if st.button("🌐 Traducir este heurístico al español"):
                    st.session_state.traducir_actual = True
                    st.rerun()
